...
- callback-heavy MLIR APIs are not yet emitted through the same plain
  autogenerated trampoline path as ordinary wrappers
...
- but they are no longer only a flat reason atom
  ...
  - and `Kinda.Wrapper.Generate` now exposes a callback-bridge audit report
    surface
...
The old `beaver` blocker set was firmly in the second category; updating the
kind surface alone would not emit those APIs. That is now resolved by a
handwritten callback-bridge runtime that merges those functions back into the
same unified declaration surface instead of leaving them as a forever blocker
bucket.

Current audit status:

- the declaration-side single-source refactor is materially landed
- `beaver` no longer carries callback-bridge generation blockers in
  [Beaver.MLIR.WrapperPolicy](/Users/tsai/oss/beaver/lib/beaver/mlir/wrapper_policy.ex:1)
- the callback-heavy MLIR CAPIs are now backed by a handwritten bridge runtime
  but emitted through the same unified declaration contract as the ordinary
  generated wrappers
- the current extracted-function audit for `beaver` is zero-missing: every
  wrapper name in `generated/capi_functions.ex` is present in the resolved
  declaration surface
- `beaver` still emits build-time audit artifacts for this seam:
  - `generated/capi_callback_bridge_manifest.json`
  - `generated/capi_callback_bridge_report.txt`
  - but they are now expected to stay empty and act as a regression tripwire
...
## Callback-Bridge Runtime Slice

`beaver` no longer generation-blocks the callback-heavy MLIR CAPI subset.
Instead, that family now lands through handwritten callback-bridge NIFs that
are merged back into the same unified declaration surface as the ordinary
generated wrappers.

That set should not be treated as an untyped blacklist forever. The right
classification remains:

- callback-heavy, runtime-backed declarations
...
- `beaver` now reports zero callback-bridge generation blockers
- callback-bridge manifests and reports are still emitted, but they are
  expected to stay empty and therefore act as regression tripwires
